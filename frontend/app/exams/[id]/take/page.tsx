...
export default function TakeExamPage() {
  ...
  }, [token, attemptId, router, initialize]);
  ...
  const fetchAttempt = async () => {
    try {
      ...
      showAlert({
        message: err.response?.data?.message || "İmtahan yüklənərkən xəta baş verdi",
        type: "error",
  ...
  const handleTimeExpired = async () => {
    ...
    showAlert({
      message: "İmtahan müddəti bitdi! Cavablar avtomatik olaraq yadda saxlanılacaq.",
      type: "warning",
      confirmButtonText: "Tamam",
    });
    setTimeout(async () => {
  ...
  const handleVisibilityChange = () => {
    if (document.hidden) {
      setTabChangeCount((prev) => {
        ...
        if (newCount >= 3) {
          showAlert({
            message: "XƏBƏRDARLIQ: Tab dəyişdirmək qadağandır! 3 dəfədən çox tab dəyişdirsəniz, imtahan avtomatik olaraq bitiriləcək.",
            type: "warning",
  ...
  const handleSubmit = async (skipConfirm: boolean = false) => {
    if (!skipConfirm) {
      const confirmed = await showConfirm({
        message: "İmtahanı bitirmək istədiyinizə əminsiniz? Dəyişiklik edə bilməyəcəksiniz.",
        type: "warning",
  ...
  const submitExam = async () => {
    ...
    try {
      ...
      showAlert({
        message: err.response?.data?.message || "İmtahan təqdim olunarkən xəta baş verdi",
        type: "error",
        confirmButtonText: "Tamam",
      });
    } finally {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            exam.topics.map((topic: any, topicIndex: number) => {
              ...
              return (
                <div key={topic.id || topicIndex} className="space-y-4">
                  ...
                  {topicQuestions.map((question: Question, qIndex: number) => {
                    const globalIndex = getAllQuestions().findIndex(q => q.id === question.id);
                    return (
                      <div
                        key={question.id}
        ...
        <div className="mt-8 flex justify-center">
          ...
          >
            {submitting ? "Təqdim olunur..." : "İmtahanı bitir və nəticəni gör"}
          </button>
        </div>
      </div>
    </div>
