...
export default function ExamsPage() {
  ...
  const { user, token, initialize, initialized } = useAuthStore();
  ...
  const fetchExams = useCallback(async () => {
    try {
      ...
      // API interceptor already handles 401 and redirects to login
      // Don't redirect here if it's already a 401 (interceptor handles it)
  ...
  useEffect(() => {
    ...
    // Check token after initialization
    if (!token) {
      router.push("/login");
      return;
    }
    ...
    fetchTeachers();
    fetchExams();
  }, [token, initialized, router, fetchExams]);
  ...
  }, [selectedTeacherId, token, initialized, loading, fetchExams]);
